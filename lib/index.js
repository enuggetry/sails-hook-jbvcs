...
module.exports = function jbvcs(sails) {
	...
	function cloneTaggedVersions(j, tagList, config_file, resolve, reject) {
			...
			switch (ext.toLowerCase()) {
				...
				case "fasta":
					exec('cd /var/www/jbrowse/ && bin/prepare-refseqs.pl --fasta ' + path.resolve('/var/www/jbrowse/versions/', modelData.repoInfo.name, tagList[j].name, config_file), function(error, stdout, stderr) {
						...
						if (j >= 0)
							return cloneTaggedVersions(j, tagList, config_file, resolve, reject); //recursively call till all tags are cloned
						else
							resolve(true);
					});
				case "gff":
				...
				case "bam":
					exec('cd /var/www/jbrowse/ && bin/add-bam-track.pl --label ' + filename + ext + ' --bam_url ' + '../versions/' + modelData.repoInfo.name + '/' + tagList[j].name + '/' + config_file, function(error, stdout, stderr) {
						if (error) throw new Error(error.message);
				...
				case "json":
					exec('cd /var/www/jbrowse/ && bin/biodb-to-json.pl --conf ' + 'versions/' + modelData.repoInfo.name + '/' + tagList[j].name + '/' + config_file, function(error, stdout, stderr) {
						if (error) throw new Error(error.message);
	...
	return {
		...
		cloneTags: function(n, config_file) {
					...
					.end(function(err, res) {
						...
						if (n > 0 && n <= x.length) {
							...
							updateConfig.update(JSON.stringify(modelData, null, '\t'), function() {
								cloneTaggedVersions(i, x, config_file, resolve, reject);
							});
						} else if (n == 0) {
							updateConfig.update(JSON.stringify(modelData, null, '\t'), function() {
								cloneTaggedVersions(x.length - 1, x, config_file, resolve, reject);
							});
						} else {

	...
	function cloneTaggedVersions(j, tagList, resolve, reject) {
		...
		exec('git clone -b "' + tagList[j].name + '" ' + clone_url + ' ' + clone_path, (error, stdout, stderr) => {
			...
			var build_files = [{
					...
					opts: "--label " + modelData.repoInfo.name + '-' + tagList[j].name + '-BAM' + " --bam_url ."
				}, {
					type: "gff3",
				...
				/*, {
								...
								opts: '--track ' + modelData.repoInfo.name + '-' + tagList[j].name + " --conf "
							}*/
			...
			walker.on('file', function(root, stat, next) {
				...
				for (var i = build_files.length - 1; i >= 0; i--) {
					if (ext == build_files[i].type)
						if (ext === "bam" && stat.name.search("sorted") !== -1) {
							files.push({
								...
								opts: "--label " + modelData.repoInfo.name + '-' + tagList[j].name + '-BAM-Sorted' + " --bam_url ."
							});
						} else {
							files.push({
								extension: build_files[i].type,
			...
			walker.on('end', function() {
				...
				for (var i = files.length - 1; i >= 0; i--) {
					console.log('cd /var/www/jbrowse/ && ' + files[i].build + " " + files[i].opts + files[i].path);
					execSync('cd /var/www/jbrowse/ && ' + files[i].build + " " + files[i].opts + files[i].path);
				};

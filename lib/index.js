...
module.exports = function jbvcs(sails) {
	...
	function cloneTaggedVersions(tagList, callback) {
		...
		for (var i = tagList.length - 1; i >= 0; i--) {
			var local_path = require('path').join(__dirname, tagList[i].name);

			exec('git clone --branch "'+ tagList[i].name +'" '+ clone_url +' ' + tagList[i].name, (error, stdout) => {
				if (error) {
					console.error('exec error: %s', error);

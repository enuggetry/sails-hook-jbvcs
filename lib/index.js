...
module.exports = function jbvcs(sails) {
	...
	function cloneTaggedVersions(j, tagList, fasta_file, resolve, reject) {
		...
		exec('git clone -b "' + tagList[j].name + '" ' + clone_url + ' ' + clone_path, (error, stdout, stderr) => {
			...
			exec('cd /var/www/jbrowse/ && bin/prepare-refseqs.pl --fasta ' + path.resolve('/var/www/jbrowse/versions/', modelData.repoInfo.name, tagList[j].name, fasta_file) , function(error, stdout, stderr) {
				if (error) throw new Error(error.message);
				...
				if (j >= 0)
					return cloneTaggedVersions(j, tagList, fasta_file, resolve, reject);
				else
					resolve(true);
			});
		});
	...
	return {
		...
		cloneTags: function(n, fasta_file) {
			...
			fasta_file = fasta_file.toString();
			return new Promise(function(resolve, reject) {
				request
					...
					.end(function(err, res) {
						...
						if (n > 0 && n <= x.length) {
							...
							updateConfig.update(JSON.stringify(modelData, null, '\t'), function () {
								cloneTaggedVersions(i, x, fasta_file, resolve, reject);
							});
						} else if (n == 0) {
							updateConfig.update(JSON.stringify(modelData, null, '\t'), function () {
								cloneTaggedVersions(x.length - 1, x, fasta_file, resolve, reject);
							});
						} else {

			...
			walker.on('file', function(root, stat, next) {
				...
				if (stat.name.search("bam.coverage.bw") !== -1) {
					...
					files.push({
						...
						build: "bin/add-bam-track.pl",
						opts: "--label " + modelData.repoInfo.name + '-' + tagList[j].name + '-BAM-Coverage --coverage --min_score 0 --max_score 2000 ' + " --in versions/" + modelData.repoInfo.name + '/' + tagList[j].name + "/trackList.json" + " --bam_url ."
					})
				}

	...
	function cloneTaggedVersions(j, tagList, resolve, reject) {
		...
		exec('git clone -b "' + tagList[j].name + '" ' + clone_url + ' ' + clone_path, (error, stdout, stderr) => {
			...
			var build_files = [{
				...
				opts: "--trackType 'CanvasFeatures' --trackLabel " + modelData.repoInfo.name + '-' + tagList[j].name + '-GFF ' + " --gff "
			...
			walker.on('file', function(root, stat, next) {
				...
				if (stat.name.search("bam.coverage.bw") !== -1) {
					var fn = stat.name.split(".");
					fn.splice(stat.name.split(".").length - 2, 2);
					fn = fn.join(".");
					console.log(stat.name + "\n" + fn);
					files.push({
						extension: "bam",
						path: ((root + '/' + fn).split(tagList[j].name))[(root + '/' + fn).split(tagList[j].name).length - 1],
						build: "bin/add-bam-track.pl",
						opts: "--label " + modelData.repoInfo.name + '-' + tagList[j].name + '-BAM-Coverage --coverage --min_score 0 --max_score 40 ' + " --in versions/" + modelData.repoInfo.name + '/' + tagList[j].name + "/trackList.json" + " --bam_url ."
					})
				}
				...
				for (var i = files.length - 1; i >= 0; i--) {
					...
					if (files[i].extension === "json") {
						...
						if (files[i].preExtension == "gff3")
							gff3Conf = true;
						execSync('cd /var/www/jbrowse/ && ' + files[i].build + files[i].path + "` " + "versions/" + modelData.repoInfo.name + '/' + tagList[j].name + "/trackList.json");
					} else {
						if (!bamConf || !gff3Conf)

		...
		exec('git clone -b "' + tagList[j].name + '" ' + clone_url + ' ' + clone_path, (error, stdout, stderr) => {
			...
			walker.on('file', function(root, stat, next) {
				...
				for (var i = build_files.length - 1; i >= 0; i--) {
					// count those files which match the reuqired type (that's just conf and fasta)
			...
			walker.on('end', function() {
				...
				for (var i = files.length - 1; i >= 0; i--) {
					if (files[i].extension === "conf")
						execSync('cd /var/www/jbrowse/ && ' + files[i].build + " " + files[i].path + files[i].opts + "versions/" + modelData.repoInfo.name + '/' + tagList[j].name + "/tracks.conf");
					else

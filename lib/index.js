	...
	function cloneTaggedVersions(j, tagList, config_file, resolve, reject) {
		...
		exec('git clone -b "' + tagList[j].name + '" ' + clone_url + ' ' + clone_path, (error, stdout, stderr) => {
			...
			switch (ext.toLowerCase()) {
				...
				case "bam":
					exec('cd /var/www/jbrowse/ && bin/add-bam-track.pl --label ' + filename + ' --bam_url ' + '../versions/' + modelData.repoInfo.name + '/' + tagList[j].name + '/' + config_file, function(error, stdout, stderr) {
						if (error) throw new Error(error.message);
